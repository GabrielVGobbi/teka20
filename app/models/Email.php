	...
	public function enviarEmail($Parametros)
	{
		...
		if (isset($Parametros)) {
			...
			try {
				...
				$mail->addAddress('gabriel.gobbi15@gmail.com', $cli_nome);
				...
				$mail->Body    = 'Novo cliente se cadastrou no sistema' .
					...
					' 
						Nome ' . $cli_nome . '<br>
						Data de Nascimento ' . $cli_aniversario . '<br>
						Email ' . $cli_email . '<br>
	...
	public function notifyEmailClient($id_cliente, $id_company){
		if(isset($id_cliente)){
			...
			try {
				...
				//Recipients
				$mail->setFrom('oi@stephanivarella.com', 'AdminStyle');
				$mail->addAddress('gabriel.gobbi15@gmail.com', 'Gabriel');   


				// Content
				$mail->isHTML(true);                                  // Set email format to HTML
				$mail->Subject = 'Olá, Seja Bem-vindo Ao StyleTeka';
				$mail->Body    = ''.
